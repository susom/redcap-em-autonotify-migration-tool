...
/**
 * Class AutoNotify3
 * COPIED FROM PLUGIN METHODS
 * @package Stanford\AutonotifyMigrationTool
 */
...
//		logIt(__FUNCTION__, "DEBUG");
        ...
        $sql = "SELECT l.sql_log, l.ts
			...
			-- AND l.page = 'PLUGIN'
			AND l.description = '" . AutoNotify3::PluginName . " Config'
...
//                logIt($this->record . ": Proj {$this->project_id} : Logic: " . json_encode($logic), "DEBUG");
                if (LogicTester::evaluateLogicSingleRecord($logic, $this->record, null, $this->project_id)) {
                    ...
                    if ($this->project_id == 9206) //logIt(__LINE__ . " Title: $title / Scope: $scope", "DEBUG");
...
//         'stanford' => 'Stanford (wrap body in nice Stanford theme html message)'
    ...
    public function checkForPriorNotification($title, $scope=0) {
		...
		// if ($this->project_id == 9206) logIt(__LINE__ . " Running: $sql", "DEBUG");
        ...
        while ($row = db_fetch_assoc($q)) {
			...
			// if ($this->project_id == 9206) logIt(__LINE__ . " Pairs: " . json_encode($pairs), "DEBUG");
    ...
    // GET ALL AUTONOTIFY ALERT LOGS FOR ALL TRIGGERS
    public function getAllNotifications() {
        ...
        while ($row = db_fetch_assoc($q)) {
            $pairs = parseEnum($row['data_values']);
            $date = substr($row['ts'], 0, 4) . "-" . substr($row['ts'], 4, 2) . "-" . substr($row['ts'], 6, 2);
            $time = substr($row['ts'], 8, 2) . ":" . substr($row['ts'], 10, 2) . ":" . substr($row['ts'], 12, 2);
            $pairs['last_sent'] = $date . " " . $time;
            $pairs['event_id'] = !empty($pairs['event']) ? \REDCap::getEventIdFromUniqueEvent($pairs['event']) : null;
            $result[] = array_intersect_key($pairs, array_flip(array("title","record","event_id","last_sent")));
    ...
    public function checkForUpgrade() {
        ...
        if (!empty($params['an'])) {
            ...
            $this->saveConfig();
            $log_data = array(
                'action' => PluginName . ' config moved from querystring to log',
                'an' => $an,
                'config' => $old_config
            );
...
//        $trigger['bcc'] = '';
    ...
    public function saveConfig() {
        ...
        // \Plugin::log("Save Config", $this->config, $sql_log);
        REDCap::logEvent(AutoNotify3::PluginName . " Config", "Configuration Updated", $sql_log, null, null, $this->project_id);
    ...
    private function callDets($urls) {
        ...
        foreach ($dets as $det_url) {
            ...
            if ($project_id = 11416) {
                // logIt("DET URL: " . $det_url);
                // logIt("POST: " . json_encode($_POST));
            }

            $result = http_post($det_url, $_POST, 10);
            // logIt("POST RESULT:".print_r($result,true));
        }
    }
    ...
    public function getDetUrl() {
        ...
        $isHttps = true; //default to https

        // logIt("SERVER HTTPS: " . @$_SERVER['HTTPS']);


        if (!empty($_SERVER['HTTPS']) AND $_SERVER['HTTPS'] == 'off') $isHttps = false;

        // logIt("IS HTTPS: " . (int) $isHttps);
    ...
    public function isDetUrlNotAutoNotify($update = false) {
        ...
        if ($data_entry_trigger_url !== $det_url) {
            ...
            if ($update) {
                ...
                db_query($sql);
                REDCap::logEvent(AutoNotify3::PluginName . " Update", "Converted DET Url to $det_url (see log table for old value)", $data_entry_trigger_url, null, null, $this->project_id);
    ...
    public function renderTrigger($id, $trigger) {
        ...
        $html = RCView::div(array('class'=>'round chklist trigger','idx'=>"$id"),
            ...
            RCView::table(array('cellspacing'=>'5', 'class'=>'tbi'),
                ...
                self::renderTestRow($id,'Test', $test_record, $test_event, 'test').
                                        ...
                                        RCView::tr(array(),
                                                RCView::td(array('style'=>'padding:10px 0'),
                                                        RCView::table(array('style'=>'font:normal 12px Arial;color:#666'),
                                                                ...
                                                                ($this->longitudinal ? (RCView::tr(array(),
                                                                        RCView::td(array('style'=>'text-align:right'),
                                                                                "Event"
                                                                                ).
                                                                        RCView::td(array('style'=>'padding-left:10px;color:#000'),

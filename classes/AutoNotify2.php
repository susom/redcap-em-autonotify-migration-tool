...
class AutoNotify2
{
    ...
    public function checkForUpgrade() {
        ...
        if (!empty($params['an'])) {
            ...
            REDCap::logEvent(self::PluginName . " Update", "Moved querystring config to log table", json_encode($log_data));
...
//		logIt(__FUNCTION__ . ": sql: $sql","DEBUG");
    ...
    public function saveConfig() {
        ...
        REDCap::logEvent(self::PluginName . " Config", "Configuration Updated", $sql_log);
...
//		logIt('Testing record '. $this->record . ' with ' . $logic, "DEBUG");
    ...
    public function getDetUrl() {
        ...
        $isHttps = (!empty($_SERVER['HTTPS']) AND $_SERVER['HTTPS'] == 'off' ? false : true);
    ...
    public function isDetUrlNotAutoNotify($update = false) {
        ...
        if ($data_entry_trigger_url !== $det_url) {
            ...
            if ($update) {
                ...
                db_query($sql);
                REDCap::logEvent(self::PluginName . " Update", "Converted DET Url to $det_url (see log table for old value)", $data_entry_trigger_url);
    ...
    public function notify($title, $trigger) {
        ...
        if (!$email->send()) {
            ...
            REDCap::logEvent(
                self::PluginName . " Error", "Error sending AutoNotify2 Email: " . $title,
